...
def format_listing(listing: LatestListing, *, include_image_link: bool = True) -> str:
    lines = [
    ...
    if listing.seller:
        if listing.seller_url:
            lines.append(f'<a href="{_escape(listing.seller_url)}">Продавец: {_escape(listing.seller)}</a>')
        else:
            lines.append(f"Продавец: {_escape(listing.seller)}")
    if listing.description:
        lines.extend(("", "<b>Описание</b>", _escape(listing.description)))
    if include_image_link and listing.image_url:
        lines.append(f'<a href="{_escape(listing.image_url)}">Открыть фото</a>')
...
class TelegramBot:
    ...
    async def latest(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if result.listing is None:
            ...
            return
        try:
            await self._send_listing(chat_id, result.listing)
        except TelegramError as exc:
            await update.effective_message.reply_text(f"Не удалось отправить карточку: {exc}")
    ...
    async def _send_listing(self, chat_id: int, listing: LatestListing) -> None:
        photo_sent = False
        if listing.image_url:
            try:
                await self.application.bot.send_photo(
                    ...
                    caption=f"<b>{_escape(listing.title)}</b>",
                    parse_mode=ParseMode.HTML,
                )
                photo_sent = True
            except TelegramError as exc:
                logger.info("Фото объявления не отправлено в чат %s: %s", chat_id, exc)
        await self.application.bot.send_message(
            chat_id=chat_id,
            text=format_listing(listing, include_image_link=not photo_sent),
            parse_mode=ParseMode.HTML,
    ...
    async def _notify(self, listing: LatestListing) -> None:
        for chat_id in self.database.subscribers():
            try:
                await self._send_listing(chat_id, listing)
            except Forbidden:
